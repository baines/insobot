...
const IRCModuleCtx irc_mod_ctx = {
	...
	.commands    = DEFINE_CMDS (
		[GET_QUOTE]     = CMD("q"   ) CMD("quote"   ),
		[ADD_QUOTE]     = CMD("qadd") CMD("q+"      ),
		[DEL_QUOTE]     = CMD("qdel") CMD("q-"      ) CMD("qrm"),
		[FIX_QUOTE]     = CMD("qfix") CMD("qmv"     ),
		[FIX_TIME]      = CMD("qft" ) CMD("qfixtime"),
		[LIST_QUOTES]   = CMD("ql"  ) CMD("qlist"   ),
		[SEARCH_QUOTES] = CMD("qs"  ) CMD("qsearch" ) CMD("qfind"  ) CMD("qgrep"),
		[GET_RANDOM]    = CMD("qr"  ) CMD("qrand"   ) CMD("qrandom")
	),
...
static Quote* quote_www_modify(QChan* chan, uint32_t id, const char* new_txt, ssize_t new_time){
	...
	if(new_time >= 0){
		s += snprintf(send_buf, sizeof(send_buf), "%zd", new_time);
	}
